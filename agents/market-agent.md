...
```
VERDICT: <one of: <n> competitors   |   NO SOURCES   |   STOPPED — <reason>>
wrote: <the path(s)>
```

`STOPPED — <reason>` covers the briefing-contract stops: no `target` named and
no `spec.md` to fall back on. It is not `NO SOURCES` — that one means you
searched and the web came back empty, which is a finding about the market. A
stop means you were never given a market to research. A caller grepping for
`VERDICT:` must be able to tell a clean stop from a crash: with no such line
at all, the rule it follows is to re-run you once and then report the pass as
not done — so a stop you could have stated in one line spends both rounds of
your ceiling reaching the same refusal twice. Write `wrote: (none)` when you
