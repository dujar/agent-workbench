...
```
VERDICT: <one of: READY   |   ROUND <n>, phase <p> — <k> questions queued   |   STOPPED — <reason>>
wrote: <the path(s)>
```

`STOPPED — <reason>` covers the briefing-contract stops: no idea given and no
`scout.md` to start from, a `scout.md` with nothing picked. A caller grepping
for `VERDICT:` must be able to tell a clean stop from a crash: with no such
line at all, the rule it follows is to re-run you once and then report the
pass as not done — so a stop you could have stated in one line spends both
rounds of your ceiling reaching the same refusal twice. Write `wrote: (none)`
when you stopped before writing.
